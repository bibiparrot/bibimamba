...
PYTHON_VERSIONS = ['3.12.3', '3.12.2', '3.12.1', '3.12.0', '3.11.9', '3.11.8', '3.11.7', '3.11.6', '3.11.5', '3.11.4',
                   '3.11.3', '3.11.2', '3.11.1', '3.11.0', '3.10.14', '3.10.13', '3.10.12', '3.10.11', '3.10.10',
                   '3.10.9', '3.10.8', '3.10.7', '3.10.6', '3.10.5', '3.10.4', '3.10.3', '3.10.2', '3.10.1', '3.10.0',
                   '3.9.19', '3.9.18', '3.9.17', '3.9.16', '3.9.15', '3.9.14', '3.9.13', '3.9.12', '3.9.11', '3.9.10',
                   '3.9.9', '3.9.8', '3.9.7', '3.9.6', '3.9.5', '3.9.4', '3.9.3', '3.9.2', '3.9.1', '3.9.0', '3.8.19',
                   '3.8.18', '3.8.17', '3.8.16', '3.8.15', '3.8.14', '3.8.13', '3.8.12', '3.8.11', '3.8.10', '3.8.9',
                   '3.8.8', '3.8.7', '3.8.6', '3.8.5', '3.8.4', '3.8.3', '3.8.2', '3.8.1', '3.8.0', '3.7.17', '3.7.16',
                   '3.7.15', '3.7.14', '3.7.13', '3.7.12', '3.7.11', '3.7.10', '3.7.9', '3.7.8', '3.7.7', '3.7.6',
                   '3.7.5', '3.7.4', '3.7.3', '3.7.2', '3.7.1', '3.7.0', '3.6.15', '3.6.14', '3.6.13', '3.6.12',
                   '3.6.11', '3.6.10', '3.6.9', '3.6.8', '3.6.7', '3.6.6', '3.6.5', '3.6.4', '3.6.3', '3.6.2', '3.6.1',
                   '3.6.0', '3.5.10', '3.5.8', '3.5.7', '3.5.6', '3.5.5', '3.5.4', '3.5.3', '3.5.2', '3.5.1', '3.5.0',
                   '3.4.10', '3.4.9', '3.4.8', '3.4.7', '3.4.6', '3.4.5', '3.4.4', '3.4.3', '3.4.2', '3.4.1', '3.4.0',
                   '3.3.7', '3.3.6', '3.3.5', '3.3.4', '3.3.3', '3.3.2', '3.3.1', '3.3.0', '3.2.6', '3.2.5', '3.2.4',
                   '3.2.3', '3.2.2', '3.2.1', '3.2', '3.1.5', '3.1.4', '3.1.3', '3.1.2', '3.1.1', '3.1', '3.0.1', '3.0',
...
class SingleWorker(QThread):
    ...
    def run(self):
        ...
        if not self.python_exe.exists():
            ...
            self.process = subprocess.Popen(popenargs, creationflags=subprocess.CREATE_NO_WINDOW)
            try:
        ...
        # print(f"SingleWorker run killed {self.killed}")
        if not self.killed:
            # print("SingleWorker run finished")
            self.finished.emit()
    ...
    def quit(self):
        if self.process and self.process.poll() is None:  # If the process is still running
            ...
            retcode = self.process.poll()
            self.killed = True
            # print(f"SingleWorker quit killed {self.killed}")
...
class bibimamba(QMainWindow, Ui_MainWindow):
    ...
    def run_single_processing(self):
        try:
            ...
            if Path(conda_path).is_dir() and Path(conda_path).exists():
                ...
                self.worker = SingleWorker(conda_path, python_version)
                self.progress_dialog = QProgressDialog("Processing ...", "Cancel", 0, 0, self,
                                                       Qt.WindowType.FramelessWindowHint)
                self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
                self.progress_dialog.setCancelButton(QPushButton("Cancel"))
                self.progress_dialog.canceled.connect(self.handle_canceled)
                self.progress_dialog.setAutoClose(False)
                self.progress_dialog.show()
                self.worker.finished.connect(self.single_processing_finished)
    ...
    def single_processing_finished(self):
        print('single_processing_finished')
        self.pushButton_2.setEnabled(True)
        conda_path = self.lineEdit.text()
        python_version = self.comboBox.currentText()
        python_dir = Path(
            conda_path) / f"conda_python_{'.'.join(python_version.split('.')[:2])}" / 'envs' / f"conda_python_{python_version}"
        if python_dir.exists():
            python_exe = python_dir / 'python.exe'
            linkage = f"<a href={str(python_dir.as_uri())} target='__blank'> {str(python_exe)}</a> "
        else:
            linkage = ' NOT Created. '
